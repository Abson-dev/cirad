...
#data<-"arbres_diohine_mai2018_par_Zone_OK_BON.shp" # base des arbres du parc ? Prosopis africana
...
library(tidyverse)
library(ggpubr)
Evaluation<-read.csv2("C:/Users/DELLDRAMOMO/OneDrive/cirad/EvaluationModeles") 

Evaluation<-Evaluation[,2:ncol(Evaluation)]
Evaluation$blocs<-plyr::revalue(Evaluation$blocs,c("Oui"="Avec blocs","Non"="Sans blocs"))
EvaluationF<-Evaluation %>% 
  dplyr::filter(Espece=="F. albida") %>% 
  dplyr::filter(Model=="Oui")

EvaluationF$blocs<-plyr::revalue(EvaluationF$blocs,c("Avec blocs"="B","Sans blocs"="NB"))
EvaluationF$Espece<-plyr::revalue(EvaluationF$Espece,c("F. albida"="Faidherbia albida"))
EvaluationF$Type<-plyr::revalue(EvaluationF$Type,c("bioclim"="Bioclim","mahal"="Mahal","MaxEnt"="Maxent","domain"="Domain"))
...
p + facet_grid(Espece ~Type) + theme_bw() + ylab("AUC") + xlab("") + theme(axis.text=element_text(size=18,face="bold"),
                                                                           axis.title=element_text(size=18,face="bold"),strip.text.x = element_text(size = 20),strip.text.y = element_text(size = 20,face="italic"))  
